param(
    ...
    [string[]]$PluginNames = @("confur","unity", "integrity", "tsml-for-unity", "amber", "trumpet", "reconcile", "scrutiny", "concordance", "sentinel", "confur"),
    [string]$OutputPath = (Get-Location).Path,
    [string[]]$ExcludeFolders = @("obj", "bin", ".git", ".idea", "vendor", "node_modules", "build")
)
...
if (-not [string]::IsNullOrWhiteSpace($QuickSelect)) {
    switch ($QuickSelect.ToLower()) {
		...
		"sen" { $PluginNames = @("sentinel") }
        "rc" { $PluginNames = @("reconcile") }
        "uta" { $PluginNames = @("unity", "tsml-for-unity", "amber") }
        "uit" { $PluginNames = @("unity", "integrity", "tsml-for-unity") }
        "all" { $PluginNames = @("unity", "integrity", "tsml-for-unity", "amber", "trumpet", "reconcile", "scrutiny", "concordance", "sentinel", "confur") }
        default {
